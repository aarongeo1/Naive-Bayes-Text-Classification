...
def calculate_confusion_matrix(test_data, predictions, classes):
...
def calculate_precision_recall(cm):
    ...
    for label in cm.columns:
        ...
        fp = cm.sum(axis=0)[label] - tp  # Sum over column
        fn = cm.sum(axis=1)[label] - tp  # Sum over row
        precision[label] = tp / (tp + fp) if (tp + fp) != 0 else 0
        recall[label] = tp / (tp + fn) if (tp + fn) != 0 else 0
    return precision, recall

def calculate_micro_macro_averages(cm, precision, recall):
...
def main():
    ...
    test_data['PredictedClass'] = predictions

    # Evaluating the model
    classes = train_data['relation'].unique()
    cm = calculate_confusion_matrix(test_data['relation'], predictions, classes)
    precision, recall = calculate_precision_recall(cm)
    micro_precision, micro_recall, macro_precision, macro_recall = calculate_micro_macro_averages(cm, precision, recall)
